...
class LargeScaleTransferLearning:
    def __init__(self, configuration: TransferLearningConfiguration):
        ...
        self.model = mm.DecoratorModel.load_from_file(
        ...
        self._logger = TransferLearningLogger(
            self.config.logging_path, self.config.with_weights
        )
    ...
    def load_sets(self, set_path):
        ...
        for path in it.cycle(file_paths):  # stores the path instead of the set
            # DEBUG dataloading:
            yield list(
                self._reader.read_library_design_data_file(path, num_fields=2)
            )

    def run(self):
        # setup
        optimizer = torch.optim.Adam(
            self.model.network.parameters(), lr=self.config.learning_rate.start
        )
